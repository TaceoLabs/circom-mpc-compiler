...
Merces is intentionally different: batch sizes 1 and 4 have no zkeys or proof workflow. Their
local-feature-gated tests compare Plain and three-party Rep3 complete witnesses, then check a
canonical SHA-256 witness digest independently generated by pinned Circom. Developers can refresh
those checked-in digests with `scripts/refresh-merces-witness-kats.sh`; CI only consumes them.
