...
// Two precomputation sites of the *same kind* that cannot be serviced together: the second one's
...
// same way), and it is what "staged precomputation" exists for: an implementation that ran every
// batch up front, or that keyed batches on multiplicative depth alone, gets this wrong.
//
...
template StagedPrecomputation() {
    signal input a;
    signal input b;
    signal output out;

    signal p <== a * b;
    signal z <== TACEO_PRECOMPUTATION_IsZero()(p);
    signal q <== z * a;
    out <== TACEO_PRECOMPUTATION_IsZero()(q);
}

component main = StagedPrecomputation();
