...
/// `TACEO_PRECOMPUTATION_Doubler` wraps `Doubler`, a gadget this compiler has no precomputation
/// implementation for. Its body is deliberately pure `Add`/`Sub`/`Mul`, so this succeeds rather
/// than failing deeper on some unrelated gap - exactly the situation the vendored
