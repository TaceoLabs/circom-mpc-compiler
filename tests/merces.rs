...
//!
//! With `artifacts/<main>/witness/<scenario>.wtns` present (see `scripts/gen-merces-artifacts.sh`)
//! each scenario additionally compares against circom's own witness. With `inputs/zkey/<main>.arks.zkey`
//! present (the merces ceremony proving key - too large to commit, see `.gitignore`), a scenario
//! produces and verifies a real co-groth16 proof. Both skip with a message rather than failing when
//! the artifact is absent, so `cargo test` stays green on a fresh clone.
...
/// `artifacts/<main>/witness/<scenario>.wtns`, generated by `scripts/gen-merces-artifacts.sh`.
fn golden_witness(main: &str, scenario: &str) -> Option<Vec<Fr>> {
    let path = Path::new(manifest_dir())
        .join("artifacts")
        .join(main)
        .join("witness")
        .join(format!("{scenario}.wtns"));
    match std::fs::File::open(&path) {
        Ok(file) => Some(
            circom_types::Witness::<Fr>::from_reader(file)
...
fn witness_extension_agrees(main: &str, scenario: &str) {
    ...
    match golden_witness(main, scenario) {
        Some(golden) => assert_eq!(
            plain, golden,
            "{main}/{scenario}: witness must match circom's own"
        ),
        None => eprintln!(
            "note: {main}/{scenario}: no golden witness - skipping that comparison. \
...
/// `snarkjs wtns check` against the same scenario already confirmed the inputs satisfy the R1CS (see
/// `scripts/gen-merces-artifacts.sh`).
    ...
    Groth16::<Bn254>::verify(&vk, proof, &public[1..]).unwrap_or_else(|e| {
        panic!(
            "{main}/{scenario}: the proof did not verify: {e}\n\
             `snarkjs wtns check` already confirmed this scenario's inputs satisfy the R1CS, so this \
             points at this compiler's witness (layout or a gadget), not at fixtures::MERCES_SCENARIOS."
        )
    });
}
