//! Verifies accelerator-gadget recognition: `Poseidon2`, `Num2Bits`, `IsZero` and `AliasCheck` are
//! each cut into exactly one `ir::AcceleratorSite` with the expected shape, matched by their own
//! circom name regardless of what (if anything) wraps them - except (for
//! `accelerator_unsupported_gadget_test`'s `Doubler`) an unrecognized name, which is not
//! recognized at all.
...
const WRAPPERS: &[Wrapper] = &[
    Wrapper {
        circuit: "accelerator_poseidon2_test",
        inner_name: "Poseidon2",
        kind: AcceleratorKind::Poseidon2 { t: 3 },
        num_inputs: 3,
        num_outputs: 3,
    },
    Wrapper {
        circuit: "accelerator_num2bits_test",
        inner_name: "Num2Bits",
        kind: AcceleratorKind::Num2Bits { n: 8 },
        num_inputs: 1,
        num_outputs: 8,
    },
    Wrapper {
        circuit: "accelerator_iszero_test",
        inner_name: "IsZero",
    ...
    Wrapper {
        circuit: "accelerator_aliascheck_test",
        inner_name: "AliasCheck",
        kind: AcceleratorKind::AliasCheck,
        num_inputs: 254,
        num_outputs: 0,
    },
];
...
fn extract_yields_one_site_per_wrapper() {
    for w in WRAPPERS {
        ...
        assert_eq!(
            sites.len(),
            1,
            "{}: expected exactly one accelerator site",
            w.circuit
        );
        let site = &sites[0];
        assert!(
...
/// `Doubler`, a gadget this compiler has no accelerator implementation for. Its body is
...
fn unrecognized_gadget_compiles_its_body() {
    let graph = CoCircomCompiler::parse(
        circuit_path("accelerator_unsupported_gadget_test"),
        config(),
    )
    .expect("an unrecognized gadget must compile its body");
    assert!(
        graph.accelerator_sites().is_empty(),
...
fn signal_span_matches_independent_total() {
    for w in WRAPPERS {
        let graph = CoCircomCompiler::parse(circuit_path(w.circuit), config())
            .unwrap_or_else(|e| panic!("{}: {e}", w.circuit));
        let site = &graph.accelerator_sites()[0];
...
fn extract_mode_runs_end_to_end_through_the_plain_driver() {
    ...
    // whole VM pipeline (codegen -> Opcode::Accelerator dispatch -> the main instruction stream) runs to
...
///
/// See `circuits/accelerator_staged_test.circom` and `passes::mpc::level`.
#[test]
fn chained_same_kind_sites_are_staged_into_separate_batches() {
    let graph = CoCircomCompiler::parse(circuit_path("accelerator_staged_test"), config()).unwrap();
    let summary = graph.mpc_summary();
    assert_eq!(summary.accelerator_sites, 2);
    assert_eq!(
        summary.accelerator_batches, 2,
...
fn independent_same_kind_sites_share_one_batch() {
    let graph = CoCircomCompiler::parse(circuit_path("accelerator_iszero_test"), config()).unwrap();
    let summary = graph.mpc_summary();
    assert_eq!(summary.accelerator_sites, 1);
    assert_eq!(summary.accelerator_batches, 1);
}

#[test]
fn num2bits_zero_returns_an_empty_trace_without_panicking() {
    let program =
        CoCircomCompiler::compile(circuit_path("accelerator_num2bits_zero_test"), config())
            .unwrap();
...
fn all_public_gadgets_stay_public_through_downstream_multiplication() {
    ...
    let graph =
        CoCircomCompiler::parse(circuit_path("accelerator_public_test"), cfg.clone()).unwrap();
    let summary = graph.mpc_summary();
    ...
    assert!(program.statistics().public_accelerator_results > 0);
