...
use circom_mpc_program::{Bank, BatchKind, Opcode, PrecomputeKind, Program, WitnessSource};
...
fn round_trips_a_staged_multi_batch_program() {
    ...
    assert_eq!(
        read_back
            .instructions()
            .iter()
            .filter(|i| i.op == Opcode::Precompute)
            .count(),
        2
    );
...
fn validation_rejects_out_of_range_witness_and_batch_targets() {
    ...
    let mut invalid_batch = program("precomputation_iszero_test").into_parts();
    ...
    let mut invalid_poseidon = program("precomputation_poseidon2_test").into_parts();
    invalid_poseidon.precompute_batches[0].kind =
        BatchKind::Precompute(PrecomputeKind::Poseidon2 { t: 5 });
