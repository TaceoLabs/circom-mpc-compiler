...
use super::build::{GadgetInstance, SubGraphInstance, TemplateGraph, TemplateOp};
use crate::ir::{self, GadgetId, GadgetSite, Op, SignalIdx, ValueId};
...
///
/// `gadget_sites` accumulates one entry per recognized gadget instance encountered anywhere in
...
pub(super) fn inline_template(
    ...
    gadget_sites: &mut Vec<GadgetSite>,
...
/// compiled body, or (for a recognized gadget) turn it into a gadget site instead of
/// compiling anything - see [`inline_gadget`].
///
...
fn inline_sub_graph_instance(
    ...
    match instance {
        ...
        SubGraphInstance::Gadget(site) => inline_gadget(
            nodes,
            outputs,
            gadget_sites,
            site,
...
/// Turns one recognized gadget instance into an `Op::Gadget` node plus one
/// `Op::GadgetResult` per result slot, instead of recursing into a compiled body. Result
...
fn inline_gadget(
    ...
    // Cross-checks the circuit's actual signal layout against what the gadget's VM implementation
    // is prepared to produce. A mismatch (a widened AliasCheck, a Num2Bits site with
    // intermediates) is a compile-time panic naming the discrepancy, not a silently truncated or
    // garbage witness.
    let actual_results = num_outputs + num_intermediates;
    let expected = kind.expected_results();
    assert_eq!(
        actual_results, expected,
        "gadget component `{header}` has {actual_results} result slots (signal layout), \
    ...
    let gadget_id = ValueId::new(nodes.len());
    nodes.push(ir::Node::new(Op::Gadget(site_id), inputs));
