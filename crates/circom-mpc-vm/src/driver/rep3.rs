...
/// deliberately one-shot: construct a new [`Self::new`] for every [`crate::Vm::rep3`]. Consuming
/// `Vm::run` enforces that at the type level - there is no reuse to reject at runtime.
...
impl<'a, N: Network> Rep3Driver<'a, N> {
    ...
    /// mask pool fails.
    pub fn new(net: &'a N, state: &'a mut Rep3State, program: &Program) -> eyre::Result<Self> {
